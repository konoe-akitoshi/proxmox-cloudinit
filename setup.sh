...
qm set ${VM_ID} --scsi0 ${DISK_POOL}:0,discard=on,import-from=/root/${UBUNTU_CODE_NAME}-server-cloudimg-amd64.img
...
# Set EFI disk
qm set ${VM_ID} --efidisk0 ${DISK_POOL}:1,efitype=4m,pre-enrolled-keys=1
# convert to template
qm template ${VM_ID}
